...
These nine cover the MicroDots framework (Effect v4 + Foldkit custom elements)
end to end: **plan** (`port-prototype`), **build** (`port-app`, `port-feature`,
`new-micro`), **prove** (`verify`, `debug-blank`), **ship** (`deploy`), and **tell**
(`content`, `brand-recap`). All of them **resolve the workspace before acting** — dot
